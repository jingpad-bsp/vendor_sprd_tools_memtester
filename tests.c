...
int test_bitspread_comparison(ulv *bufa, ulv *bufb, size_t count) {
    ...
    for (j = 0; j < UL_LEN * 2; j++) {
        ...
        for (i = 0; i < count; i++) {
            if (j < UL_LEN) { /* Walk it up. */
                *p1++ = *p2++ = (i % 2 == 0)
                    ? (ONE << j) | (ONE << ((j + 2)%UL_LEN))
                    : UL_ONEBITS ^ ((ONE << j)
                                    | (ONE << ((j + 2)%UL_LEN)));
            } else { /* Walk it back down. */
                *p1++ = *p2++ = (i % 2 == 0)
                    ? (ONE << (UL_LEN * 2 - 1 - j)) | (ONE << ((UL_LEN * 2 + 1 - j)%UL_LEN))
                    : UL_ONEBITS ^ (ONE << (UL_LEN * 2 - 1 - j)
                                    | (ONE << ((UL_LEN * 2 + 1 - j)%UL_LEN)));
            }
        }
        //printf("\b\b\b\b\b\b\b\b\b\b\b");
        //printf("testing %3u", j);
...
void switch_for_tiger_64(ulv *p){
    char *r0;
    char *r1;
    char new_r0[8],new_r1[8];
    ulv *p_r = p;
    int i; 
#define copy_byte(__x,__y) do{\
    __y = __x;}while(0)
    ...
    copy_byte(r1[2],new_r1[0]);
    copy_byte(r1[3],new_r1[2]);
    copy_byte(r0[4],new_r0[5]);
    copy_byte(r0[5],new_r0[7]);
    copy_byte(r0[6],new_r1[5]);
    copy_byte(r0[7],new_r1[7]);
    copy_byte(r1[4],new_r0[4]);
    copy_byte(r1[5],new_r0[6]);
    copy_byte(r1[6],new_r1[4]);
    copy_byte(r1[7],new_r1[6]);
    for(i = 0; i < 8; i++){
        r0[i] = new_r0[i]; /*write back*/
        r1[i] = new_r1[i];
    }
...
int test_bitflip_comparison_tiger(ulv *bufa, ulv *bufb, size_t count) {
    ...
    for (k = 0; k < UL_LEN; k++) {
	    ...
	    for (i = 0; i < count; i++) {
		*p1++ = *p2++ = (i % 2) == 0 ? q : ~q;
                if ((i % 2) == 1){  /*switch for tiger*/
                    if (sizeof(ulv) == 4) {
			switch_for_tiger(p1-1); 
                	switch_for_tiger(p2-1); 
                    } else if (sizeof(ulv) == 8) {
			switch_for_tiger_64(p1-1); 
                	switch_for_tiger_64(p2-1); 
		    } else {
		        printf("This test only support for 32bit and 64 bit Architecture");
		        return -1;
		    }
		}
	    }
...
int compare_regions_const(ulv *bufa, size_t count, int value) {
	...
	for(i = 0; i < count; i++, p1++) {
		if(*p1 != value) {
			printfTag(TAG_REFRESH, "FAILURE: 0x%08lx != 0x%08x at offset 0x%08lx.\n",
